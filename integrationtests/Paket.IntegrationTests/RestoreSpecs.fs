...
[<Test>]
let ``#AGR2325 paket.lock newer than per-project paket.props forces per-project restore``() =
    ...
    // the mtime-based invalidation that fixes that case.
    let project = "project"
    let scenario = "iAGR2325-lock-mtime-invalidates-project-props"
    use __ = prepareSdk scenario
